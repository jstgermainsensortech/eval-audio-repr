...
class TorchMLPClassifier2:
    ...
    def fit(self, X, y, X_val=None, y_val=None, val_idxs=None):
        ...
        return _train_model(self.device, self.model, criterion, self.optimizer, None, train_loader, eval_loader, metric=metric,
    ...
    def score(self, test_X, test_y, classes=None):
        ...
        val_loss, targets, preds = _validate(self.device, self.model, eval_loader, criterion)
    ...
    def save(self, folder_path):
        path = os.path.join(folder_path, "weight_params.pth")
        ...
        torch.save({
            ...
            'oh_enc': self.oh_enc,
            'config': {k: v for k, v in self.__dict__.items() if k in params}
        }, path)
        print(f"Model saved to {path}")

    @staticmethod
    def load(folder_path, test_X, test_y):
        path = os.path.join(folder_path, "weight_params.pth")
        ckpt = torch.load(path, map_location='cpu', weights_only=False)

        instance = TorchMLPClassifier2(**ckpt['config'])
        instance.standard_scaler = ckpt['standard_scaler']
        instance.oh_enc = ckpt['oh_enc']

        metric, criterion, n_class, test_y, _ = instance.switch_regime(test_y, None)

        instance.model = MLP(input_size=test_X.shape[-1], hidden_sizes=instance.hidden_layer_sizes, output_size=n_class)
